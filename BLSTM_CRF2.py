import torch
from helper import argmax, log_sum_exp, pos_sent
...
class BLSTM_CRF2(nn.Module):

    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, add_features_dim, debug=False):
        super(BLSTM_CRF2, self).__init__()
        self.debug = debug
        ...
        '''
        self.all_feats_dim = self.hidden_dim + add_features_dim
    ...
    def _add_features(self, lstm_out, add_features):
        ...
        if self.debug: print("features size: ", add_features.size())
        feats = torch.cat((lstm_out, add_features), 1)
        return feats


    def _get_emission(self, sentence, add_features):
        if self.debug: print("** inside get_emission **")
        lstm_out = self._get_lstm_hidden(sentence)
        all_feats = self._add_features(lstm_out, add_features)
    ...
    def forward(self, sentence, add_features):
        ...
        emission = self._get_emission(sentence, add_features)
