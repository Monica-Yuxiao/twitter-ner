...
class CRF(nn.Module):
    ...
    def __init__(self, n_dice, log_likelihood):
        ...
        print("** initializing loglikelihood **")
    ...
    def _data_to_likelihood(self, rolls):
        ...
        """
        print("** inside _data_to_likelihood **")
        return Variable(torch.FloatTensor(self.loglikelihood[rolls]), requires_grad=False)
        # print("rolls likelihood size: ", rolls_likelihood.size())
        return rolls_likelihood
    ...
    def _compute_likelihood_numerator(self, loglikelihoods, states):
        ...
        print("** inside numerator **")
        prev_state = self.n_states
        score = Variable(torch.Tensor([0]))
        # print("initializing score: ", score)
        for index, state in enumerate(states):
            # print("self.transition[state, prev_state]:\n", self.transition[state, prev_state])
            # print("loglikelihoods[index, state]:\n", loglikelihoods[index, state])
    ...
    def neg_log_likelihood(self, rolls, states):
        ...
        """
        # print("before _data_to_likelihood: \n", rolls)
        loglikelihoods = self._data_to_likelihood(rolls)
        # print("after _data_to_likelihood: \n", loglikelihoods)
    ...
    def forward(self, rolls):
        print("** forward function get called **")
        loglikelihoods = self._data_to_likelihood(rolls)
